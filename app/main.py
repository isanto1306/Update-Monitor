...
# v0.3.366 frontend bridge source. The release synchronization script copies
# this small script into the large static index.html. It keeps manual update
# progress alive through short status requests after the backend has accepted
# the long Docker operation, avoiding reverse proxy HTTP 504 timeouts.
...
# v0.3.372 frontend error-localization bridge. Backend messages intentionally
...
(function(){
  ...
  function localizePayload(value,key=''){
    ...
    if(typeof value==='object'){
      ...
      for(const [childKey,childValue] of Object.entries(value)){
        copy[childKey]=localizePayload(childValue,childKey);
      }
      return copy;
    }
  ...
  window.updateMonitorLocalizeBackendMessage=localizeBackendMessage;

  api=async function(url,options={}){
    try{
      const result=await baseApi(url,options);
      return localizePayload(result);
    }catch(error){
      if(!error||String(error.message||'')==='auth')throw error;
      const translated=localizeBackendMessage(error.message,true);
      if(translated===String(error.message||''))throw error;
      const wrapped=new Error(translated);
