...
## Webhook + Registry update detection

Update Monitor can use a hybrid update detection mode. For installed apps that resolve to a GitHub repository, it can automatically register a signed repository webhook when the configured GitHub token has Webhooks write permission and Update Monitor is being accessed through a public HTTPS address. A GitHub package publication then triggers a targeted check of only the affected app. Apps that cannot use a webhook continue to use the existing registry checks, and the normal scheduled full check remains the safety net.

Webhook deliveries never install an update by themselves. They only request a targeted Update Monitor scan. Incoming GitHub payloads are verified with X Hub Signature 256 before they are accepted.
