...
Current release: **v0.3.375**

v0.3.375 also treats a manual per-app Docker check as an explicit fresh version lookup. Before the targeted scan starts, only that application's GitHub release and digest-to-version cache entries are invalidated, allowing an already-updated rolling image to refresh a stale displayed release immediately.\n\nv0.3.374 invalidates only the affected application's GitHub release catalogue and digest-to-version cache after a successful Docker/Compose update, before the targeted verification scan. This prevents a newly installed rolling image from remaining labeled with the previous release while unrelated app caches stay intact.\n\nv0.3.373 extends backend and API error localization to all five existing UI languages: German, English, French, Portuguese and Spanish. It also restores a generated five-language translation audit so every release checks dictionary and placeholder parity across the actual runtime translations.\n\nv0.3.372 localizes backend and API error messages before they are inserted into translated UI text. German mode no longer mixes a German prefix with raw English backend errors, while English mode keeps the original technical message. The same central localization path also covers action progress and stored automatic-update errors.\n\nv0.3.371 removes the experimental GitHub webhook automation again. Update detection is Registry only, which avoids unnecessary webhook permissions and keeps the existing scheduled Registry checks as the single reliable mechanism. The backup progress improvements from v0.3.368 remain unchanged.\n\nv0.3.368 smooths the visible backup percentage between backend-confirmed progress values. The browser never advances beyond the latest real backend percentage, fast Docker copy jumps are interpolated visually, and a real 100% backup result stays visible before installation begins.\n\nv0.3.367 makes full-backup progress continuous while Docker data is being copied. Update Monitor uses measured copied bytes when a fast size sample is available and otherwise shows a conservative bounded intermediate estimate; visible changes are limited to small steps, and the copy phase can never reach completion until `docker cp` actually exits.\n\nv0.3.366 separates backup progress from installation progress: a backup now completes its own 0–100% phase before installation begins. Manual update installation is also dispatched to a backend worker and followed through short action-progress requests, so long Docker operations are no longer tied to one proxy request that can be reported as HTTP 504.
